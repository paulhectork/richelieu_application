<template>

  <div class="article-header">
    <h1>Le mobilier urbain</h1>
    <h2>Reflet d'un quartier et de ses mobilités</h2>
  </div>

  <div class="article-body">
    <p>Depuis le XIX<sup>e</sup> siècle, le quartier Richelieu fait office d'épicentre
      des politiques urbaines relatives au
      <RouterLink to="/theme/qr1b56ce7199bc94c7db5cfbcbb2849220d">
        mobilier urbain</RouterLink>.
      Popularisée par JC Decaux dans les années 1960, cette notion revêt une
      définition administrative, soit
      <q>&nbsp;l'ensemble des objets ou dispositifs publics installés dans
        <RouterLink to="/theme/qr1bc9332b2db5f45ee9dec8433307f8e7f">
          l'espace public</RouterLink>&nbsp;
                <UiButtonEllipsis data-key="thezy1976"></UiButtonEllipsis>&nbsp;
      </q>.
      À cette définition se superposent un volet légal, comme vecteur de normes, et un
      aspect symbolique, comme témoin et faiseur d'urbanité.
    </p>
    <p>Centre névralgique de la capitale, ce quartier a la particularité d'être
      autant fréquenté de jour que de nuit, conférant de ce fait à l'éclairage
      une grande importance. La rue Vivienne fait partie des trois premières rues
      dotées de lanternes au gaz à la fin des années 1820
      <UiButtonEllipsis data-key="nouveau1829"></UiButtonEllipsis>.
      L'effet de ces lanternes se voit démultiplié par la pose
      de candélabres dans les années 1840
      <UiButtonEye data-key="qr116f7dbcdb2884ccd8ea4160c07202ad3"></UiButtonEye>.
      La volonté d'assurer un confort visuel optimal et un sentiment de sécurité
      nocturne aux citadins demeure une priorité municipale jusqu'à la fin des
      années 1920
      <UiButtonEllipsis data-key="libre1929"></UiButtonEllipsis>.
    </p>
    <p>Une autre activité phare du quartier exerce une influence sur le
      mobilier de rue&nbsp;: le
      <RouterLink to="/theme/qr1ccc4169a977f41a995771d81522941e2">
        théâtre</RouterLink>.
      Le préfet Rambuteau (1781-1869)
      choisit d'implanter les premières colonnes urinoirs à proximité des
      théâtres parisiens, le long du boulevard des Italiens et du boulevard
      Montmartre. Loin d'être de simples lieux d'aisance, elles servent aussi
      de support pour la
      <RouterLink to="/theme/qr1dda4429d380a4244ab6c8873e41c6fa1">
        publicité</RouterLink>
      , en atteste une lithographie de F. Bouchot
      <UiButtonEye data-key="qr14be40957a31c4767b80c3de152d094a0"></UiButtonEye>.
      Rapidement jugées incongrues, elles laissent place, à partir
      de 1868, aux colonnes Morris. Ces dernières sont légion dans ce quartier
      propice au divertissement. Initialement conçues pour la réclame théâtrale,
      elles présentent également des
      <RouterLink to="/theme/qr1a5a5ddd859b4463f96c2ec2e7ba23686">
        affiches</RouterLink>
      en lien avec l'alimentation
      <UiButtonEye data-key="qr1a98e4dd3d2b7432da0971c604bf34cc1"></UiButtonEye>&nbsp;
      <RouterLink to="/article/menu">
        <UiButtonArrow orient="right"></UiButtonArrow>
      </RouterLink>.
    </p>
    <p>Le jardin du Palais-Royal sert aussi de moteur en matière d'équipement
      public. Ce lien étroit entre l'aménagement des rues et des espaces verts
      est principalement le fait de l'architecte Gabriel Davioud (1824-1881).
      Il a impulsé, par son travail sur les promenades
      <UiButtonEye data-key="qr17ecadb767ccf4f80a131f84daea1696b"></UiButtonEye>,
      toute une série de modifications, notamment au sujet des urinoirs.
      Cet architecte dote les pissotières de stalles multiples et d'écrans
      afin de masquer ce que la destination a de vulgaire. On retrouve alors
      place de la Bourse un urinoir grillagé à six stalles
      <UiButtonEye data-key="qr1d15d16273e194973a295b618fb812540"></UiButtonEye>,
      immortalisé par Charles Marville (1813-1879). Parangon
      de la modernité haussmannienne, cet édicule permet de retracer l'histoire
      des
      <RouterLink to="/theme/qr1d8b0b911dd7b43ff825c908263e1fcc1">
        mœurs</RouterLink>
      de ce quartier. Il est définitivement fermé en 1903
      <UiButtonEllipsis data-key="bulletin1903"></UiButtonEllipsis>
      à cause de sa réputation interlope. Haut lieu des amours homosexuelles,
      encore prohibées à l'époque, les témoignages abondants
      <UiButtonEllipsis data-key="raffeqeau1884"></UiButtonEllipsis>
      confèrent à ce quartier une place de choix dans la géographie queer
      parisienne de la seconde moitié du XIX<sup>e</sup> siècle.
    </p>
    <p>Enfin, l'étude du mobilier urbain contribue à décliner la figure du flâneur
      au féminin. Des femmes de tous milieux se côtoient dans ce quartier, principalement
      sur la place de la Bourse. Si elles s'y rendent pour des raisons variées
      (travail, achat, divertissement, etc.), toutes sont égales face à l'urgence
      d'un besoin vésical. Les femmes voient leur prière exaucée lors de l'inauguration
      du premier chalet de nécessité
      <UiButtonEye data-key="qr1df69056471584f02bf3200a819d3d7cb"></UiButtonEye>
      le 26 septembre 1873, place de la Bourse
      <UiButtonEllipsis data-key="petite1873"></UiButtonEllipsis>.
      Malgré de timides débuts, les chalets finissent par être un succès
      populaire, si bien qu'on en dénombre plus d'une centaine dans Paris au début
      du XX<sup>e</sup> siècle.</p>
    <p>Le mobilier urbain, loin de se cantonner à <q>&nbsp;un vieux réverbère à souvenirs
      au coin d'une rue où il ne passe déjà presque plus personne
      <UiButtonEllipsis data-key="celine1932"></UiButtonEllipsis>&nbsp;
    </q>, pour reprendre
      les termes de L-F. Céline, enfant maudit du quartier, est un excellent indicateur
      des mutations d'une ville et dans le cas présent, d'un quartier.
    </p>
  </div>

  <div class="article-footer">
    <p>Louise Thiroux</p>
  </div>

</template>


<script setup>
import { onMounted, ref } from "vue";

import UiButtonEye from "@components/UiButtonEye.vue";
import UiButtonArrow from "@components/UiButtonArrow.vue";
import UiButtonEllipsis from "@components/UiButtonEllipsis.vue";
import { urlToFrontendTheme, urlToFrontendNamedEntity } from "@utils/url.js";

/************************************************/

const rootUrl     = window.location.origin;
const emit        = defineEmits(["queryParams", "iiifIdUuidArray", "footnotes"]);
const queryParams = { theme: [ "espace public"
                             , "mobilier urbain"
                             , "affiche"
                             , "théâtre"
                             , "publicité"
                             , "mœurs"
                             ],
                      themeBooleanOp: "or" };
const iiifIdUuidArray = [ "qr116f7dbcdb2884ccd8ea4160c07202ad3"  // MC55
                        , "qr14be40957a31c4767b80c3de152d094a0"  // MC315
                        , "qr1d15d16273e194973a295b618fb812540"  // BHVP23
                        , "qr1df69056471584f02bf3200a819d3d7cb"  // MAK1
                        , "qr17ecadb767ccf4f80a131f84daea1696b"  // PRBNF24
                        , "qr1a98e4dd3d2b7432da0971c604bf34cc1"  // BDFPM119
                        ]
const footnotes = {
  "thezy1976"    : `Marie de Thézy, <i>Paris, la rue. Le mobilier urbain parisien
                    du Second Empire à nos jours à travers les collections
                    photographiques de la Bibliothèque historique de la ville de
                    Paris</i>, Paris, Société des Amis de la Bibliothèque historique,
                    1976, p. 7.`,
  "nouveau1829"  : `<i>Nouveau journal de Paris et des départements&nbsp;: feuille
                    administrative, commerciale, industrielle et littéraire</i>,
                    16 avril 1829, p. 2.`,
  "libre1929"    : `<i>La libre parole républicaine</i>, 18 avril 1929, p. 2.`,
  "bulletin1903" : `<i>Bulletin municipal officiel de la Ville de Paris</i>, Paris,
                    Imprimerie municipale, 17 janvier 1903, p.394.`,
  "raffeqeau1884": `Donatien Raffegeau, <i>Du Rôle des anomalies congénitales des
                    organes génitaux dans le développement de la folie chez l'homme</i>,
                    Paris, A. Davy, 1884, p. 16.&nbsp;; <i>La Justice</i>, 09
                    septembre 1888, p. 2.`,
  "petite1873"   : `<i>La Petite Presse&nbsp;: journal quotidien</i>, 27 septembre
                    1873, p. 4.`,
  "celine1932"   : `Louis-Ferdinand Céline, <i>Voyage au bout de la nuit</i>, Paris,
                    Denoël et Steele, 1932, p. 566.`

};


/************************************************/

/************************************************/

// when the component is mounted, send base emits to parent
onMounted(() => {
  emit("queryParams", queryParams);
  emit("iiifIdUuidArray", iiifIdUuidArray);
  emit("footnotes", footnotes)
})
</script>


<style scoped>

</style>
